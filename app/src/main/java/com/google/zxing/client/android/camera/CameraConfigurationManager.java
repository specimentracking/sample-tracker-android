...
//    }
    ...
    private Point findBestPreviewSizeValue(Camera.Parameters parameters, Point screenResolution) {
        ...
        for (Camera.Size supportedPreviewSize : supportedPreviewSizes) {
            ...
            // 1196 x 768 is not very standard aspect ratio and closest one
            // from preview sizes is in most cases 720 x 480, but this has very bad results
            // when scanning the 128C barcode. So for this resolution we will try to force better preview,
            // despite the fact that the screen image will be little bit narrowed.
            boolean isProblematicResolution = screenResolution.x == 1196 && screenResolution.y == 768;
            if (isProblematicResolution) {
                if ((realWidth == 1280 && realHeight == 720) ||
                    (realWidth == 800  && realHeight == 480)) {
                    bestSize = new Point(realWidth, realHeight);
                    Log.i(TAG, "For problematic resolution 1196x768 better resolution has been forced: " + bestSize);
                    break;
                }
            }
